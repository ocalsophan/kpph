...
Begin VB.Form frm_Ebupotpph23 
   ...
   End
End
Attribute VB_Name = "frm_Ebupotpph23"
...
Function generate_sql() As String
    ...
    sql = "Select ID, NPWP_KPP, Kode_Proyek, " & _
        "No_Bukti_Akuntansi, Jenis_Dokumen, Tgl_Dokumen_ddMMyyyy, " & _
        "No_Faktur_Pajak, Kode_Form_Bukti_Potong, Masa_Pajak, " & _
        "Tahun_Pajak, Pembetulan, NPWP_WP_yang_Dipotong, " & _
        "NIK_Yg_Dipotong, Nomer_telepon, Kode_Objek_Pajak, " & _
        "Penanda_tangan_BP_Pengurus, Mendapatkan_Fasilitas, Nomor_SKB, " & _
        "Nomor_Aturan_DTP, NTPN_DTP, Nama_WP_yang_Dipotong, " & _
    ...
    If Not (Trim(Me.cb_npwpKpp) = "" Or Trim(Me.cb_npwpKpp) = "ALL") Then
        If Trim(kondisi) = "" Then
            kondisi = "NPWP_KPP = '" & Trim(Me.cb_npwpKpp) & "' "
        Else
            kondisi = kondisi & " and NPWP_KPP = '" & Trim(Me.cb_npwpKpp) & "' "
        End If
    End If
    ...
    If Trim(Me.txt_cari.text) <> "" Then
        cari = "NPWP_KPP like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                ...
                "Tgl_Dokumen_ddMMyyyy like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "NPWP_WP_yang_Dipotong like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "NIK_Yg_Dipotong like '%" & Trim(Me.txt_cari.text) & "%' or " & _
    ...
    sql = sql & " order by Tahun_Pajak desc, Masa_Pajak, Kode_Proyek, Tgl_Dokumen_ddMMyyyy "
...
Private Sub cmd_ubah_Click()
    ...
    For a = start_kolom To 91
        If a <= 23 Or a >= 90 Then
            val1(a) = InputBox(rs.Fields(a).Name, "Input", cek_null(rs.Fields(a).Value))
        End If
    Next
    
    p = MsgBox("Jadi Ubah data " & nama_data & "?", vbYesNo)
    If p = vbNo Then Exit Sub
    
    sql = "update ebupot23 set "
    For a = start_kolom To 91
        If a <= 23 Or a >= 90 Then
            If a = 91 Then
...
Private Sub Form_Load()
  ...
  sql = "select distinct Tahun_Pajak from ebupot23"
  Call Load_combo(Me.cb_tahun, sql, cnn, True, , 1)
  
  sql = "select distinct Masa_Pajak from ebupot23"
  Call Load_combo(Me.cb_masa, sql, cnn, True, , 1)
  
  sql = "select distinct kode_divisi from ebupot23"
  Call Load_combo(Me.cb_divisi, sql, cnn, True, , 1)
  
  sql = "select distinct NPWP_KPP from ebupot23"
  Call Load_combo(Me.cb_npwpKpp, sql, cnn, True, , 1)
