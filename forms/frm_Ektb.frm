...
Begin VB.Form frm_Ektb 
   ...
   Begin VB.Label lb_caption 
      ...
      Caption         =   "Ekualisasi : Data Trial Balance (Data digunakan untuk Ekualisasi) - Posisi SALDO s/d bulan XX"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Menu mnData 
      ...
      Begin VB.Menu mnRekao 
         Caption         =   "Rekap Per Proyek"
      End
      Begin VB.Menu mnRekap2 
         Caption         =   "Rekap 1 Perusahan"
      End
   End
End
Attribute VB_Name = "frm_Ektb"
...
Function generate_sql() As String
    ...
    Dim cari As String, limit As Integer
    
    'kondisi
    kondisi = ""
    cari = ""
        
    sql = "Select id1, tahun, bulan, " & _
        "kode_proyek_lama, kode_proyek_baru, kode_akun, " & _
        "deskripsi, nama_proyek, debit, " & _
        "kredit, Icon " & _
        "From all2016_tb"
    ...
    If Trim(Me.txt_cari.text) <> "" Then
        cari = "bulan like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "kode_proyek_baru like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "kode_akun like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "deskripsi like '%" & Trim(Me.txt_cari.text) & "%' or " & _
    ...
    sql = sql & " order by tahun desc,bulan, kode_proyek_lama, kode_proyek_baru "
        
    limit = cek_Int(InputBox("Jumlah data yang ditampilkan? 0 : semua", "", "0"))
        
    If limit <> 0 Then
        sql = sql & " limit " & limit
        Me.Frame3.Caption = " " & kondisi & " / " & Trim(Me.txt_cari.text) & " - Limit " & limit
    Else
...
Private Sub cmd_add_Click()
    ...
    sql = "insert into all2016_tb(" & _
            "tahun, bulan, kode_proyek_lama, " & _
            "kode_proyek_baru, kode_akun, deskripsi, " & _
            "nama_proyek, debit, kredit, " & _
            "icon) values ("
    For a = start_kolom To rs.Fields.Count - 1
...
Private Sub Form_Load()
  ...
  sql = "select distinct kode_proyek_lama from all2016_tb"
...
Private Sub mnRekap2_Click()
    frm_Grid.Show
    frm_Grid.sql = "select tahun, bulan, " & _
                    "sum(debit), sum(kredit), if(sum(debit) = sum(kredit),'ok','') as isBalance " & _
                    "From all2016_tb " & _
                    "group by tahun, bulan "
    frm_Grid.judul = "Data TB"
