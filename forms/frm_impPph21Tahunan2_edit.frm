...
Begin VB.Form frm_impPph21Tahunan2_edit 
   ...
   Begin VB.Label lb_caption 
      ...
      Caption         =   "Import PPh 21 Tahunan (2/untuk akhir tahun) - EDIT"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frm_impPph21Tahunan2_edit"
...
Private Sub cmd_import_Click()
    ...
    Dim jRec As Long, c As Long, jml_Update As Long, hasil As Integer, mod1 As Long
    ...
    Dim Ptkp As String, id1 As String
    
    Dim Insentif As Currency, Thr As Currency, Lainnya As Currency
    ...
    Do While rs.EOF = False
        c = c + 1
        Call info(1, "Run Import " & c & " of " & jRec & ". Jml Update: " & jml_Update, Me.StatusBar1)
        mod1 = c Mod 5000
        ...
        If data_Valid = True Then
            hasil = tbPph21Tahunan2_edit2(No1, Bulan, Tahun, npwp_kpp, kdPROYEK, kdCENTER, nama, npwp, NIK _
                                            , alamat, Jabatan, P_L, Ptkp, id1, Insentif, Thr, Lainnya)
        
            If hasil = 1 Then
                jml_Update = jml_Update + 1
            Else
                Me.List1.AddItem "Data ke " & c & " error update"
                Me.List1.ListIndex = Me.List1.ListCount - 1
            End If
        End If
        rs.MoveNext
    ...
    MsgBox "Proses Import Selesai. Jml Update: " & jml_Update, vbInformation
    Me.Enable_Form
    Exit Sub
er1:
    MsgBox Err.Description, vbCritical
...
Private Sub cmd_info_Click()
    ...
    t = "data diletakkan di EXCEL, menggunanan 'Template Awal PPh Pasal 21' " & vbCr & _
        ...
        "Iuran Pensiun/Potongan Lain. " & vbCr & _
