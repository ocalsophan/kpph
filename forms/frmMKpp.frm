...
Begin VB.Form frmMKpp 
   ...
   Begin VB.Shape Shape1 
      ...
      Width           =   10575
   End
   Begin VB.Menu mnImpKpp 
      Caption         =   "Import Data KPP"
   End
End
Attribute VB_Name = "frmMKpp"
...
Private Sub cmdUbah_Click()
    ...
    npwp = cleanStr(rsGrid(0))
    nama = cleanStr(rsGrid(1))
    alamat = cleanStr(rsGrid(2))
    tgl_lahir = cek_Date(rsGrid(3))
    klu = cleanStr(rsGrid(4))
    nip_nama_ar = cleanStr(rsGrid(5))
    status_update = cleanStr(rsGrid(6))
    tgl_update = cek_Date(rsGrid(7))
    kpp_administrasi = cleanStr(rsGrid(8))
    
    nama = cleanStr(InputBox("Nama", "Update", nama))
    alamat = cleanStr(InputBox("alamat", "Update", alamat))
    tgl_lahir = cek_Date(InputBox("tgl_lahir", "Update", tgl_lahir))
    klu = cleanStr(InputBox("KLU", "Update", klu))
    nip_nama_ar = cleanStr(InputBox("NIP_Nama_AR", "Update", nip_nama_ar))
    status_update = cleanStr(InputBox("Status_Update", "Update", status_update))
    tgl_update = cek_Date(InputBox("Tgl_Update", "Update", tgl_update))
    kpp_administrasi = cleanStr(InputBox("KPP_Administrasi", "Update", kpp_administrasi))
...
Sub load_grid(Optional sql As String = "")
    ...
    If Trim(sql) = "" Then
        sql = "select npwp, nama, alamat, " & _
                "tgl_lahir, klu, nip_nama_ar, " & _
                "status_update, tgl_update, kpp_administrasi " & _
...
Private Sub txtCari_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        sql = "select npwp, nama, alamat, " & _
                ...
                "status_update, tgl_update, kpp_administrasi " & _
                "from mkpp " & _
                "where npwp like '%" & Trim(Me.txtCari) & "%' " & _
                "or nama like '%" & Trim(Me.txtCari) & "%' " & _
                "or nip_nama_ar like '%" & Trim(Me.txtCari) & "%' " & _
                "or alamat like '%" & Trim(Me.txtCari) & "%' " & _
                "or kpp_administrasi like '%" & Trim(Me.txtCari) & "%' order by npwp"
        Call load_grid(sql)
    End If
...
Private Sub cmdHapus_Click()
    ...
    If p = vbNo Then
        ...
        Exit Sub
    Else
        If tbMKpp_Delete(key1) = True Then
            Call load_grid
        Else
