...
Begin VB.Form frm_EkMas_Acc 
   ...
   Begin VB.Label lb_caption 
      ...
      Caption         =   "Ekualisasi : Chart of Account"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frm_EkMas_Acc"
...
Function generate_sql() As String
    ...
    sql = "Select id1, account, accpac, acct_name " & _
            "From all2016_maccount "
    
    '-- ini sql cari
    If Trim(Me.txt_cari.text) <> "" Then
        cari = "account like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "accpac like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "acct_name like '%" & Trim(Me.txt_cari.text) & "%' "
    End If
    ...
    generate_sql = sql & " order by account"
...
Private Sub cmd_hapus_Click()
    ...
    Dim account As String, id1 As String, acct_name As String
    Dim p
    ...
    For j = 0 To Me.DataGrid1.SelBookmarks.Count - 1
        ...
        id1 = cek_null(rs(0))
        account = cek_null(rs(1))
        acct_name = cek_null(rs(3))
        p = MsgBox("Yakin menghapus 1 record data untuk " & vbCr & "Account: " & account & vbCr & _
                    "Nama : " & acct_name & vbCr & "?", vbYesNo)
        If p = vbYes Then
            isAdaYangDihapus = True
            sql = "delete from all2016_maccount where id1 = '" & id1 & "'"
            If ExecSQL1(cnn, sql) <> 0 Then
...
Private Sub cmd_ubah_Click()
    ...
    sql = "update all2016_maccount set "
    For a = 2 To rs.Fields.Count - 1
...
Private Sub mnDataDouble_Click()
    frm_Grid.Show
    frm_Grid.sql = "select account, count(*) as jumlah " & _
                    "From all2016_maccount " & _
...
Private Sub mnHapusAll_Click()
    ...
    If p = vbYes Then
        sql = "delete from all2016_maccount"
        If ExecSQL1(cnn, sql) <> 0 Then
                sql = InputBox("error", "", sql)
        End If
        Call LoadGrid
