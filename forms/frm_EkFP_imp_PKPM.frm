...
Begin VB.Form frm_EkFP_imp_PKPM 
   ...
   Begin VB.Label lb_caption 
      ...
      Caption         =   "Ekualisasi PPn / PPh : Import FP Keluaran dan Masukan"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frm_EkFP_imp_PKPM"
...
Private Sub cmd_import_Click()
    ...
    Dim kode_divisi As String, kode_proyek_lama  As String, kode_proyek_baru As String
    Dim tahun As String, tgl_fp As Date, no_fp  As String, dpp As Currency, ppn As Currency
    Dim KETERANGAN  As String
    'kolom tambahan
    Dim pk_pm As String, masa As String, npwp_rekanan As String
    Dim nama_rekanan As String, kode_fp As String
    
    
    Dim hapus_data As Boolean, p
    ...
    Do While rs.EOF = False
        ...
        'ssssssssssssssssss
        kode_divisi = cleanStr(rs(2))
        kode_proyek_lama = cleanStr(rs(3))
        kode_proyek_baru = cleanStr(rs(4))
        tahun = cleanStr(rs(7))
        masa = cleanStr(rs(6))
        If hapus_data = True Then
            p = MsgBox("Hapus data untuk tahun/masa/divisi " & tahun & "/" & _
                        masa & "/" & kode_divisi & "?", _
                        vbYesNo)
            If p = vbYes Then
                Call pesan2("data tahun/masa/divisi: " & tahun & "/" & masa & _
                        "/" & kode_divisi & " akan di hapus terlebih dahulu")
            ...
            End If
        End If
        tgl_fp = cek_Date(cleanStr(rs(5)))
        no_fp = cleanStr(rs(11))
        dpp = cek_Money(rs(12))
        ppn = cek_Money(rs(13))
        KETERANGAN = (cleanStr(rs(14)))
        pk_pm = cleanStr(rs(1))
        npwp_rekanan = cleanStr(rs(8))
        nama_rekanan = cleanStr(rs(9))
        kode_fp = cleanStr(rs(10))
     
        If Trim(tahun) = "" Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " Tahun tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf Trim(masa) = "" Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " masa tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf Not (UCase(Trim(pk_pm)) = "PK" Or UCase(Trim(pk_pm)) = "PM") Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " pk/pm tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf Trim(kode_proyek_lama) = "" Or Trim(kode_proyek_baru) = "" Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " kode proyek lama/baru tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf tbAll2016_master_isKodeProyekLamaValid(kode_proyek_lama) = False Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " kode proyek lama/baru tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf Trim(no_fp) = "" Then
            data_Valid = False
        ...
        If data_Valid = True Then
            ret1 = tbAll2016_fp_insert2(kode_divisi, kode_proyek_lama, kode_proyek_baru, _
                                    tahun, tgl_fp, no_fp, dpp, ppn, KETERANGAN, _
                                    False, pk_pm, masa, npwp_rekanan, nama_rekanan, _
                                    kode_fp)
            If ret1 = "" Then
...
Private Sub cmd_info_Click()
    ...
    On Error GoTo er1
    MsgBox "Data FP di import per tahun / masa / divisi!!", vbInformation
    File1 = "explorer.exe " & App.Path & "\rep\template_imp_fp_pkpm.xls"
