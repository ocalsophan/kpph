...
Begin VB.Form frm_SAP_PPh_Imp 
   ...
   End
End
Attribute VB_Name = "frm_SAP_PPh_Imp"
...
Private Sub cmd_import_Click()
    ...
    Dim document_number As String, year_month1 As String, posting_key As String
    Dim document_header_text As String, account As String, acct_name As String
    Dim profit_center As String, document_date As String, posting_date As String
    Dim text As String, reference As String, amount_in_lc As Currency
    Dim vendor As String, name_cust_ven As String, reversed_with As String
    Dim cost_center As String, user_name As String, entry_date As Date
    ...
    Do While rs.EOF = False
        ...
        'ssssssssssssssssss
        document_number = cleanStr(rs(0))
        year_month1 = cleanStr(rs(1))
        posting_key = cleanStr(rs(2))
        document_header_text = cleanStr(rs(3))
        account = cleanStr(rs(4))
        acct_name = cleanStr(rs(5))
        profit_center = cleanStr(rs(6))
        document_date = cleanStr(rs(7))
        posting_date = cleanStr(rs(8))
        text = cleanStr(rs(9))
        reference = cleanStr(rs(10))
        amount_in_lc = cek_Money(cleanStr(rs(11)))
        vendor = cleanStr(rs(12))
        name_cust_ven = cleanStr(rs(13))
        reversed_with = cleanStr(rs(14))
        cost_center = cleanStr(rs(15))
        user_name = cleanStr(rs(16))
        entry_date = cek_Date(cleanStr(rs(17)))
     
        If Trim(document_number) = "" Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " document_number tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf Trim(account) = "" Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " account tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        ElseIf Trim(posting_key) = "" Then
            data_Valid = False
            Me.List1.AddItem "Data ke " & c & " posting_key tidak valid"
            Me.List1.ListIndex = Me.List1.ListCount - 1
        End If
    
        If data_Valid = True Then
            ret1 = tbPph_sap_insert(document_number, year_month1, posting_key, _
...
Private Sub cmd_info_Click()
    ...
    On Error GoTo er1
    File1 = "explorer.exe " & App.Path & "\rep\SAP_pph.xlsx"
