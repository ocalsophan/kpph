...
Begin VB.Form frm_EkBP_imp 
   ...
   Begin VB.Label lb_caption 
      ...
      Caption         =   "Ekualisasi PPn / PPh : Import Bukti Potong"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frm_EkBP_imp"
...
Private Sub cmd_import_Click()
    ...
    Dim tahun As String, tgl_BP As Date, no_BP  As String, dpp As Currency, PPh As Currency
    Dim no_FP As String, KETERANGAN As String
    ...
    Do While rs.EOF = False
        ...
        If hapus_data = True Then
            ...
            sql = "delete from all2016_bp where tahun = '" & tahun & "'"
            If ExecSQL1(cnn, sql) <> 0 Then
                sql = InputBox("sql error", "", sql)
            End If
            hapus_data = False
        End If
        tgl_BP = cek_Date(cleanStr(rs(5)))
        no_BP = cleanStr(rs(6))
        dpp = cek_Money(rs(7))
        PPh = cek_Money(rs(8))
        no_FP = (cleanStr(rs(9)))
        ...
        If data_Valid = True Then
            ret1 = tbAll2016_BP_insert(kode_divisi, kode_proyek_lama, kode_proyek_baru, _
                                    tahun, tgl_BP, no_BP, dpp, PPh, no_FP, KETERANGAN, False)
            If ret1 = "" Then
...
Private Sub cmd_info_Click()
    ...
    On Error GoTo er1
    MsgBox "Data BP di import per tahun!!", vbInformation
    File1 = "explorer.exe " & App.Path & "\rep\ekualisasi_imp_BP.xlsx"
