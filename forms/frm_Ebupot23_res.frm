...
Begin VB.Form frm_Ebupot23_res 
   ClientHeight    =   7260
   ClientLeft      =   165
   ClientTop       =   510
   ClientWidth     =   12300
   ...
   Begin VB.Label lb_caption 
      ...
      Caption         =   "Data Bukti Potong dari DJP"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frm_Ebupot23_res"
...
Function generate_sql() As String
    ...
    sql = "Select ID, JENIS_PPH, TAHUN_PAJAK, " & _
        "MASA_PAJAK, NOMOR_BUPOT, IDENTITAS_DIPOTONG, " & _
        "JENIS_IDENTITAS_DIPOTONG, NAMA_DIPOTONG, KODE_OBJEK_PAJAK, " & _
        "JUMLAH_BRUTO, PPH_DIPOTONG, STATUS_BUKTI_POTONG, " & _
        "PEMB_KE, NAMA_DOK_REF, NOMOR_DOK_REF, " & _
        "TGL_DOK_REF, STATUS_PELAPORAN  " & _
        "From ebupot23_result "
    ...
    If Trim(Me.txt_cari.text) <> "" Then
        cari = "NOMOR_BUPOT like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "IDENTITAS_DIPOTONG like '%" & Trim(Me.txt_cari.text) & "%' or " & _
                "NOMOR_DOK_REF like '%" & Trim(Me.txt_cari.text) & "%' "
    End If
    ...
    sql = sql & " order by Tahun_Pajak desc, Masa_Pajak, NOMOR_BUPOT "
...
Sub update_label()
    ...
    txt = "tahun " & Me.cb_tahun & " masa " & Me.cb_masa
...
Private Sub Form_Load()
  ...
  sql = "select distinct Tahun_Pajak from ebupot23_result"
  Call Load_combo(Me.cb_tahun, sql, cnn, True, , 1)
  
  sql = "select distinct Masa_Pajak from ebupot23_result"
...
Private Sub mnHapus_Click()
    ...
    '--- prepare
    sql = "delete from ebupot23_result"
    If Not (Trim(Me.cb_tahun) = "" Or Trim(Me.cb_tahun) = "ALL") Then
        kondisi = "Tahun_Pajak = '" & Trim(Me.cb_tahun) & "' "
    End If
...
Private Sub mnRekao_Click()
    ...
    frm_Grid.Show
    sql = "select Tahun_Pajak, MASA_PAJAK, JENIS_PPH, sum(JUMLAH_BRUTO), " & _
            "sum(PPH_DIPOTONG) " & _
            "from ebupot23_result "
    If Trim(kondisi) <> "" Then
        sql = sql & "where " & kondisi
    End If
    sql = sql & "group by Tahun_Pajak, MASA_PAJAK, JENIS_PPH"
